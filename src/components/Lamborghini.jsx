...
export function Lamborghini(props) {
    ...
    const { scene, nodes, materials } = useGLTF('/models/modified_urus.glb')
    useMemo(() => {
        ...
        Object.values(nodes).forEach((node) => {
            ...
            if (node.isGroup){
                if(node.name === 'Universal_spoiler_1'){
                    node.visible = spoiler.color === '#00' ? false : true;
                    let insideNode = node.children;
                    insideNode[0].material = new THREE.MeshPhysicalMaterial({
                        color: spoiler.color,
                    })
